...
const calculateTotalTimeline = () => { 
  ...
  if (totalUnitsValue && vacancyCreditLossValue && leaseUpToStabilizationValue) {
    leaseUpToStabilizationDurationValue = (totalUnitsValue * (1 - vacancyCreditLossValue / 100) / leaseUpToStabilizationValue) || 0;
  }
...
/*
  -------------------Disposition and Projected Returns---------------------------
*/
// inputs
const exitMonth = document.getElementById('exit-month');
const annualNoiGrowthRate = document.getElementById('annual-noi-growth-rate');
const capRateOnSale = document.getElementById('cap-rate-on-sale');
const sellingCost = document.getElementById('selling-cost');
// intermediate result
const netSalesProceeds = document.getElementById('net-sales-proceeds');
// results
const resultantUntrendedNOI = document.getElementById('resultant-untrended-noi');
const resultantExitMonth = document.getElementById('resultant-exit-month');
const resultantAnnualNoiGrowth = document.getElementById('resultant-annual-noi-growth');
const resultantNoiUponExit = document.getElementById('resultant-noi-upon-exit');
const resultantCapRateOnSales = document.getElementById('resultant-cap-rate-on-sales');
const resultantGrossSalesPrice = document.getElementById('resultant-gross-sales-price');
const resultantSellingCost = document.getElementById('resultant-selling-cost');
const resultantNetSalesProceeds = document.getElementById('resultant-net-sales-proceeds');
const resultantNetProfit = document.getElementById('resultant-net-profit');
const resultantEquityMultiple = document.getElementById('resultant-equity-multiple');
...
*/
const calculateNetSalesProceeds = () => { 
  const untrendedNOIValue = parseInt(netOperatingIncome.innerText.replace(/\,|\$/g, '')) || 0;
  const exitMonthValue = parseInt(exitMonth.value) || 0;
  const annualNoiGrowthRateValue = parseFloat(annualNoiGrowthRate.value) || 0;

  if (exitMonthValue) {
    resultantExitMonth.innerText = `${exitMonthValue} months`;
  } else {
    resultantExitMonth.innerText = `0 months`;
  }

  if (annualNoiGrowthRateValue) {
    resultantAnnualNoiGrowth.innerText = `${annualNoiGrowthRateValue}%`;
  } else { 
    resultantAnnualNoiGrowth.innerText = `0.0%`;
  }

  const noiUponExitValue = Math.round(untrendedNOIValue * Math.pow(1 + (annualNoiGrowthRateValue / 100), exitMonthValue / 12));

  if (noiUponExitValue) {
    resultantNoiUponExit.innerText = `$${numberWithCommas(noiUponExitValue)}`;
  } else {
    resultantNoiUponExit.innerText = `$0`;
  }

  const capRateOnSaleValue = parseFloat(capRateOnSale.value) || 0;

  if (capRateOnSaleValue) {
    resultantCapRateOnSales.innerText = `${capRateOnSaleValue}%`;
  } else {
    resultantCapRateOnSales.innerText = `0.0%`;
  }

  const grossSalesPriceValue = Math.round(noiUponExitValue / (capRateOnSaleValue / 100));

  if (grossSalesPriceValue) { 
    resultantGrossSalesPrice.innerText = `$${numberWithCommas(grossSalesPriceValue)}`;
  } else {
    resultantGrossSalesPrice.innerText = `$0`;
  }

  const sellingCostValue = parseFloat(sellingCost.value) || 0;
  const sellingCostAmountValue = Math.round((sellingCostValue / 100) * (-grossSalesPriceValue));

  if(sellingCostAmountValue) {
    resultantSellingCost.innerText = `$${numberWithCommas(sellingCostAmountValue)}`;
  } else {
